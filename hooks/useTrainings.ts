...
export function useTrainings() {
  ...
  const [historyTrainings, setHistoryTrainings] = useState<Training[]>([]);
  ...
  useEffect(() => {
    ...
    const loadScheduledTrainings = () => {
      try {
        ...
        if (storedScheduled) {
          const parsedScheduled: ScheduledTraining[] =
            JSON.parse(storedScheduled);
          setScheduledTrainings(parsedScheduled);
        }
      } catch (error) {
        console.error(
          "Erreur lors du chargement des scheduled trainings:",
          error
        );
      } finally {
        setIsLoading(false);
  ...
  useEffect(() => {
    const filterTraining = trainings.filter(
      (e) => e.history && e.history.length > 0
    );
    if (filterTraining) {
      setHistoryTrainings(filterTraining);
  ...
  const deleteScheduledTraining = (trainingId: string) => {
    const updatedTrainings = scheduledTrainings.filter(
      (t) => t.id != trainingId
    );
  ...
  const updateScheduledTraining = (
    trainingId: string,
    updatedTraining: ScheduledTraining
  ) => {
    const updatedTrainings = scheduledTrainings.map((t) =>
      t.id === trainingId ? updatedTraining : t
    );
    setScheduledTrainings(updatedTrainings);
    localStorage.setItem(
